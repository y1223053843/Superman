...
def execute():
    ...
    for codeItem in query_result:
        ...
        if xinhao.__contains__('V型翻转'):
            caozuo = '【操作】买入点'
            toDataFrame_param_content({}, '★★★★★My_Code_JSON_Mongo_' + time.strftime('%Y-%m-%d_%H:%M', time.localtime(time.time())) + '#' + caozuo + caozuo2 +',【主题】黄金贵金属#',xinhao + xiaomowang, collectionName)
