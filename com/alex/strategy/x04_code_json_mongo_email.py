...
##################################
'''
collectionName = "report_600891_" + time.strftime('%Y-%m-%d', time.localtime(time.time()))
...
if (param == 1):
    ...
    toDataFrame_param_content({},'★★★★★My_Code_JSON_Mongo_' + time.strftime('%Y-%m-%d_%H:%M', time.localtime(time.time())) + '#【长期关注】秋林集团' + caozuo + '#',content, collectionName)
