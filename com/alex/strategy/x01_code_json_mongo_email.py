    ...
    for codeItem in query_result:
        ...
        try:
            ...
            if (mairuresult_D !='' and (mairuresult_60  != '' or mairuresult_bl_60 != '')):
                i = 0
                for i in [0] :
                    xinhao = xinhao + codeItem + '买入信号出现：' +  mairuresult_D + '<br> ' + mairuresult_60 + '<br>' + mairuresult_bl_60 + ' '
                    email_util.sendMail(codeItem + '买入信号出现：' + mairuresult_D + '<br>' + mairuresult_60 + '<br>' + mairuresult_bl_60, codeItem + '买入，谨慎谨慎再谨慎')
                    i = i + 1
                    time.sleep(3)
...
if (param == 1):
    ...
    toDataFrame_param_content({},'★★★★★My_Code_JSON_Mongo_' + time.strftime('%Y-%m-%d_%H:%M', time.localtime(time.time())) + '#【长期关注】金马股份' + caozuo + '#',content, collectionName)
